...
from ..utils import (MultiheadAttention, SwiGLUFFNFused, build_norm_layer,
                     resize_pos_embed, to_2tuple,MultiHeadDifferentialAttention, VisualContrastAttention)
...
class TransformerEncoderLayer(BaseModule):
    def __init__(self,
        ...
        # 根据类型选择注意力机制
        if attn_type == 'diff':
            self.λ = nn.Parameter(torch.tensor(0.2))
            self.attn = MultiHeadDifferentialAttention(
                h=num_heads,
                d=embed_dims // num_heads,
    ...
    def forward(self, x, H=None, W=None):
        # 对于VCA需要传入空间尺寸
        if hasattr(self, 'attn_type') and self.attn_type == 'vca':
            x = x + self.attn(self.ln1(x), H, W)
        else:
            x = x + self.attn(self.ln1(x), self.λ)
        x = self.ffn(self.ln2(x), identity=x)
        return x


@MODELS.register_module()
class LinearDiffVisionTransformer(BaseBackbone):
    arch_zoo = {
        **dict.fromkeys(
            ['s', 'small'], {
                'embed_dims': 768,
    ...
    def __init__(self,
                 ...
                 pre_norm=False,
                 init_cfg=None):
        super(LinearDiffVisionTransformer, self).__init__(init_cfg)
    ...
    def init_weights(self):
        super(LinearDiffVisionTransformer, self).init_weights()
    ...
    def forward(self, x):
        ...
        H, W = patch_resolution  # 获取特征图空间尺寸
        ...
        for i, layer in enumerate(self.layers):
            # 传递空间尺寸给需要的注意力层
            if layer.attn_type in ['vca', 'window']:
                x = layer(x, H, W)
            else:
                x = layer(x)
