...
class CompatLayerNorm(nn.LayerNorm):
    """兼容低版本 PyTorch 的 LayerNorm，支持下标语法"""
    @classmethod
    def __getitem__(cls, *args, **kwargs):
        # 忽略下标参数，直接返回自身（兼容 nn.LayerNorm[dim] 语法）
        return cls
...
def build_norm_layer(cfg: dict, num_features: int) -> nn.Module:
    ...
    layer_type = cfg_.pop('type')
    # ---------------------- 关键修改：替换 LayerNorm 为兼容版 ----------------------
    if layer_type in ['LN', 'LayerNorm']:
        # 直接使用兼容类，不通过 MODELS 注册表（避免冲突和下标问题）
        norm_layer = CompatLayerNorm
    else:
        norm_layer = MODELS.get(layer_type)
    # ---------------------- 原有逻辑不变 ----------------------
    if norm_layer is None:
